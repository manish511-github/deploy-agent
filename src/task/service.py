...
class TaskService:
        ...
        try:
            with self._connect() as conn:
                with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                    ...
                    row = cur.fetchone()
            if row is None:
                raise DatabaseError("INSERT returned no row")
            return AgentTask(
                task_id=row["task_id"],
    ...
    def complete_task(
        ...
        """Mark a task as completed and move it to task_history."""
        update_queue_sql = """
            UPDATE task_queue
        ...
        """
        delete_queue_sql = "DELETE FROM task_queue WHERE task_id = %s"
        try:
            with self._connect() as conn:
                with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                    cur.execute(
                    ...
                    if row:
                        cur.execute(
                            ...
                            (
                                ...
                                row["task_type"],
                                row["status"],
                                psycopg2.extras.Json(row["result"])
